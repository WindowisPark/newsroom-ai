...
### F5. 초안 RAG — 자사 기사 참조 + 스타일 앵커 (차별화 심화)

**배경**: 단순 초안 생성(F3)은 일반 LLM 보고체를 내놓는다. 편집국 실무에선 (a) 자사 과거 보도 맥락을 이어가고 (b) 자사 매체로 인용해 신뢰도를 보강하며 (c) 자사 문체에 가까운 톤이 필요. RAG 학계·업계 합의(근거: docs/REFERENCES.md)를 반영한 검색-증강 초안 생성.

**파이프라인**:
```
① Retrieval — 서울신문 기사 검색
   ├ 후보: Article WHERE source_name = '서울신문'
   ├ 매칭: analysis.keywords ∩ topic_hint/articles keywords
   │       OR agenda._title_contains 재사용 (한글 경계 매칭)
   ├ Recency bias: published_at >= today - 90d 우선
   └ Reranking: score = 0.6 × (키워드 매칭 수) + 0.4 × recency
   → 상위 3건 (references[])

② Style anchor (C 보조)
   └ 동일 카테고리의 서울신문 최근 기사 1건 lead 300자
      (few-shot 톤 샘플, 주제 무관)

③ Context injection (~600 토큰 증가)
   ├ 참고 기사 3건: title + published_at + description 200자
   └ 톤 샘플 1건: lead 300자

④ DRAFT_SYSTEM 프롬프트 보강
   ├ "참고 기사에 등장한 사실만 본문에 인용하고, sources 에 포함"
   ├ "참고 기사에 없는 사실은 추측 금지 (유보적 표현 사용)"
   └ "톤 샘플의 문장 길이·어휘·구성 패턴을 참고"

⑤ Fallback
   └ 서울신문 매칭 0건 → 다른 국내 매체로 대체, references[] 비우고
     프론트에 '자사 참고 기사 없음' 안내

⑥ Transparency UX — DraftDialog 내 접힘 패널
   └ "참고한 자사 기사 (3건) ▼"
      각 카드: 제목 · 발행일 · 매체 · 원문링크 → 클릭 시 외부 열림
      style_anchor 가 있으면 별도 배지 "톤 샘플: {제목}"
```

**스키마 변경**:
- `DraftOut.references: list[SourceRef]` (검색된 자사 기사, 최대 3)
- `DraftOut.style_anchor: SourceRef | None` (톤 참고 1건, 없을 수 있음)
- `SourceRef` 에 `published_at: str | None` 추가 (recency 표시용)

**비용·품질 밸런스**:
- 참고 3건 + 톤 앵커 1건 = 평균 +600~800 토큰. Sonnet 1건당 +$0.01 내외 추정.
- references 없이 생성 시 대비 faithfulness·자사 톤 정합 모두 향상 기대 (완벽 재현은 아님 — REFERENCES §2-2)

**기대치 조정** (과장 금지):
- "서울신문 톤 그대로 재현" ❌ — implicit style 완벽 모방은 현행 LLM 한계
- "일반 LLM 보고체 → 국내 종합지 어조로 nudge" ✓ — 앵커 1건만으로도 유의미한 개선

**코퍼스 확보**:
- 상시: RSS 수집 파이프라인에 서울신문 피드 추가 (source_name 필터)
- 시연 안정성: `scripts/seed_seoul_articles.py` 로 샘플 10~20건 일회성 삽입
